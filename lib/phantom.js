...
function bundle (filePath, cb) {
    ...
    var child = exec('browserify --debug --ignore-missing -t coffeeify --extension=".coffee" --extension=".litcoffee" --extension=".coffee.md" -o '+tempFilePath+' '+templateFile)
...
function runTest (filePath, cb) {
    ...
    child.stdout.on('data', function (data) {
        try {
            parsePhantomLogs(JSON.parse(data))
        } catch (err) {
            // console.log(err)
        }
    })
