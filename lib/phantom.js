...
function bundle (filePath, cb) {
    ...
    var templateFile = wrapFileInBrowserifyTemplate(filePath)
        console.log('browserify '+templateFile+ ' --ignore-missing')
    var child = exec('browserify '+templateFile+ ' --ignore-missing')
    ...
    child.on('exit', function (code) {
        ...
        // if (code)
            // removeFile(__tempdir+tempFileDir)
...
function wrapFileInBrowserifyTemplate (filePath) {
    ...
    var template = "var Mocha = require('mocha'); var mocha = new Mocha(); require('"+templateRelativePath+"'); mocha.reporter('spec').ui('tdd').run(function (failures) { phantom.exit(); });"
